...
const latestblog = ({ latest }) => {
  return (
    <div className='bg-[#FFE3E3]'>
      ...
      <div className='lg:text-2xl md:text-2xl text-xl font-semibold'>{latest.attributes.Title}</div>
      <img
      ...
      />
      <div>
      <ReactMarkdown className='lg:mx-48 md:mx-10 text-xl'>{latest.attributes.Description}</ReactMarkdown>
...
export async function getServerSideProps({ params }) {
  ...
  try {
    ...
    const latest = latests.find(latest => latest.attributes.slug === latestslug);
